...
Generated by `run_cot_tagging.py`. Model: **claude-opus-4-8** (same as the direct tagger), Anthropic Messages API, 10 sentences per request, structured (json_schema) outputs.

Identical codebook and system prompt to the direct tagger (`05_linguistic/tag_llm.py`, `tagging_prompt.md`) --- the only difference is one added schema field, `reasoning`, which the model must fill in BEFORE `damsl`/`beads` for each sentence.

Sample: a fixed-seed (`20260806`) per-debate-proportional stratified sample of the same 24-debate, candidate-only sentence pool used throughout the paper, at 12.4% (~3,000 sentences). The DIRECT-prompt comparison numbers use the pre-existing `outputs/llm_tags.csv` results restricted to the identical sample, so both conditions are scored on exactly the same sentences.
    ...
    "results": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "reasoning": {
            "type": "string",
            "description": "1-2 sentences of reasoning about the discourse act and any rhetorical bias, written BEFORE you decide the codes."
          },
          "damsl": {
            "type": "array",
            "items": {
